import Link from "next/link";
import Image from "next/image";
import { SOCIAL_LINKS, SITE_NAME } from "@/lib/constants";
import AdminLoginLink from "./AdminLoginLink";

const socialItems = [
  {
    key: "email",
    label: "Uniquemaidcafe@gmail.com",
    href: SOCIAL_LINKS.email,
    icon: (
      <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z" />
      </svg>
    ),
    color: "text-blue-500",
  },
  {
    key: "facebook",
    label: "Unique maid cafe",
    href: SOCIAL_LINKS.facebook,
    icon: (
      <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
    color: "text-blue-600",
  },
  {
    key: "instagram",
    label: "unique.maidcafe",
    href: SOCIAL_LINKS.instagram,
    icon: (
      <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
      </svg>
    ),
    color: "text-pink-500",
  },
  {
    key: "tiktok",
    label: "uniquemaidcafe",
    href: SOCIAL_LINKS.tiktok,
    icon: (
      <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z" />
      </svg>
    ),
    color: "text-gray-900",
  },
];

export default function Footer() {
  return (
    <footer className="mt-auto border-t-4 border-pink-300 bg-white">
      <div className="lace-top mx-auto max-w-6xl px-4 py-10">
        <div className="grid gap-8 md:grid-cols-2">
          <div>
            <h3 className="mb-4 text-lg font-bold text-pink-600">עקבו אחרינו ♡</h3>
            <ul className="space-y-3">
              {socialItems.map((item) => (
                <li key={item.key}>
                  <a
                    href={item.href}
                    target={item.key === "email" ? undefined : "_blank"}
                    rel={item.key === "email" ? undefined : "noopener noreferrer"}
                    className="flex items-center gap-3 text-pink-800 transition hover:text-pink-500"
                  >
                    <span className={item.color}>{item.icon}</span>
                    <span className="font-medium">{item.label}</span>
                  </a>
                </li>
              ))}
            </ul>
          </div>

          <div className="flex flex-col items-center justify-center">
            <Link href="/" className="transition hover:scale-105">
              <Image
                src="/images/logo.png"
                alt="Unique Maid Cafe Israel logo - Maid cafe pop-up events in Israel"
                width={160}
                height={160}
                className="drop-shadow-lg"
              />
            </Link>
            <p className="mt-2 text-center text-sm text-pink-500">
              Maid & Butler — חוויה יפנית מתוקה באירועים
            </p>
          </div>
        </div>

        <div className="mt-8 flex flex-col items-center gap-3 border-t border-pink-100 pt-4">
          <nav className="flex flex-wrap items-center justify-center gap-2 text-xs text-pink-500">
            <Link href="/terms" className="hover:text-pink-700 hover:underline">
              תנאי שימוש
            </Link>
            <span aria-hidden="true">|</span>
            <Link href="/privacy" className="hover:text-pink-700 hover:underline">
              מדיניות פרטיות
            </Link>
            <span aria-hidden="true">|</span>
            <Link href="/accessibility" className="hover:text-pink-700 hover:underline">
              הצהרת נגישות
            </Link>
          </nav>
          <p className="text-xs text-pink-400">
            © {new Date().getFullYear()} {SITE_NAME}. כל הזכויות שמורות.
          </p>
          <AdminLoginLink />
        </div>
      </div>
    </footer>
  );
}
